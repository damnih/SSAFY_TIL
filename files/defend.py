        ...
        for multi in range(1, FIRE_RANGE + 1):
            ...
            for di, dj, com_key in di_dj_key:
                ...
                if 0 <= tar_i < len(map_data) and 0 <= tar_j < len(map_data):
                    if map_data[tar_i][tar_j] in ENEMY and (mega_round or normal_round):
                        is_it_real = True
                    ...
                    elif map_data[tar_i][tar_j] == 'X' and (normal_round or mega_round):
                        # 진짜 쏴도 되는지 확인하자
                        is_it_real = True
                        # 사거리 2나 3일때 그 사이를 확인한다
                        # 1일때는 쏴도 된다
    ...
    def choose_next(cur_i, cur_j):
        ...
        for di, dj, com_key in di_dj_key:
            ...
            if 0 <= next_i < len(map_data) and 0 <= next_j < len(map_data):
                ...
                if turret_position[0] < len(map_data)//2:
                    if turret_position[0] <= next_i < turret_position[0]+3:
                        continue
                    if turret_position[1]-2 <= next_j < turret_position[1]+1:
                        continue
                elif turret_position[0] >= len(map_data)//2:
                    if turret_position[0]-2 <= next_i < turret_position[0]:
                        continue
                    if turret_position[1] <= next_j < turret_position[1]+3:
                        continue
                if map_data[next_i][next_j] == 'G' or (map_data[next_i][next_j] == 'T' and mega_round):
